...
function howManyTextInPara(fullText: string, searchTerm: string): number {
    let arraySplit:number=fullText.split(searchTerm).length;
    return arraySplit -1
}
console.log(isTextInPara(paragraph,"FAMilia"));
console.log(howManyTextInPara(paragraph.toLowerCase(),"virgen".toLowerCase()));
